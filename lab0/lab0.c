...
int main(void)
{
	...
	//Variables for changing states based on whether SW1 is pressed or not
while(1)
	{

//State machine that takes the current LED and changes states
//Depending on the LED that needs to be on the state will set
//the other LEDs to OFF and set the timer for the LED that is 
//currently on.
switch(ledToToggle) {

		//As the state machine changes from one state to another
		//the LED corresponding to the next state is switched on
		//and starts blinking and all other LEDs turn off.
		case 4:
			PR1 = 14400;
			LATBbits.LATB14 = 1;
			LATBbits.LATB13 = 1;
    		LATBbits.LATB12 = 1;
					...
					//Nested in each case is an IF statement. The IF statement
					//checks if SW1 is pressed and if it is, then the timer for
					//the LED activated by the current state is set to half it's
					//default value
					if(PORTBbits.RB5 == 0) {
						PR1 = 7200;
					}
...
//LED 5 failed to blink at twice the rate when SW1 is pressed.
//The other 3 LEDs worked and the code for those other states 
//is identical in structure. The only differences in the cases 
//is that LED 5 is not set to off as the other LEDs are. Tests 
//were tried to determine if it may be a problem with the IF
//check. Alterations to the IF statement showed that the check 
//works for setting the LED to OFF or ON when SW1 is pressed. I
// could find no differences between the cases that would explain
//this issue.
		case 5:
			PR1 = 14400;
			LATBbits.LATB15 = 1;
